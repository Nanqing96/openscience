...
export async function generateIllustrationStoryboard(gateway: Pick<AiGateway, 'completeStructured'>, claims: readonly PresentationClaim[], settings: StoryboardRequest, base?: StoryboardView) {
  const sourceLookup = new Map<string, { evidenceId: string; text: string; relation: string }>();
  const upstream = claims.map(claim => {
    ...
    for (const passage of claim.sourcePassages) sourceLookup.set(`${claim.id}:${passage.evidenceId}`, passage);
    return { id: claim.id, analysis: claim.statement, conditions: claim.conditions, limitations: claim.limitations, sourcePassages: claim.sourcePassages };
  });
  const previous = base?.output === 'image' ? base.document.scenes.map(scene => {
    ...
    return { science: { title: scene.title, narration: scene.narration, message: brief.message, domain: brief.domain,
      subjects: brief.subjects.map(subject => ({ description: subject.description, basis: { claimId: subject.basis.claimId, evidenceId: subject.basis.evidenceId } })),
...
Return exactly {title,scenes:[{title,narration,message,domain,subjects,labels,constraints,encoding}]}. title/narration/message are nonempty single-line strings<=120 characters. domain: real-space|wavevector-space|time|frequency|parameter-space|conceptual. Each scene has 1–2 subjects {description:string<=100,basis:{claimId,evidenceId}}; select complete supplied original records supporting the FULL description including qualifiers. Only supports evidence can establish a subject. Other evidence remains context for limits or conflicts. Do not manufacture identifiers or quote text. Prefer a narrow supported statement over loosely related facts. labels: 0–6 exact short visible scientific strings<=80 each. constraints: 1–2 strings<=120 giving essential applicability or limits. encoding:string<=200 describes ONLY what sourced relationship each necessary mark/region/axis/arrow represents in this domain, referring to subject indices 0,1 and label indices. No unsupported mapping between domains. A logical dependency is not a physical trajectory. Title and narration may only restate the selected message/subjects. Every scientific term and condition in labels/encoding/message must be supported by a subject's basis. No new mathematical inference, formula normalization, extrema, numbers, or apparatus geometry beyond those sources. Source conflicts must not be silently resolved. Keep a single visual takeaway concise enough for about 700 characters including its later art direction.\n${scienceSkills.instructions}` },
  ...
  function materializeScience(value: unknown): { title: string; scenes: ScientificScene[] } {
    ...
    return { title: text(root.title, 120), scenes: root.scenes.map(raw => {
      ...
      const subjects = scene.subjects.map(rawSubject => {
        ...
        const basis = object(subject.basis); keys(basis, ['claimId', 'evidenceId']);
        const original = sourceLookup.get(`${basis.claimId}:${basis.evidenceId}`);
        ...
        return { description: subject.description, basis: { claimId: basis.claimId, evidenceId: original.evidenceId, quote: original.text } };
      });
  ...
  }, scienceMessages, { temperature: 0.1, maxRetries: 1, validationFeedback: () => `Correct this scientific-intent field: ${diagnostic}. Keep one narrow supported relationship, encoding<=200 characters; use complete original evidence identifiers, not quoteId or fabricated quotations. If the compiled prompt exceeds its limit, reduce optional labels or scope while preserving essential qualifiers; leave room for art direction.` });
  ...
  }, artMessages, { temperature: 0.3, maxRetries: 1, validationFeedback: () => `Art direction failed: ${diagnostic}. Return only scenes/layout/treatment. Shorten layout and treatment to fit; science fields cannot be edited.` });
