...
function collectCandidates(req: FastifyRequest): Candidate[] {
  ...
  if (path === '/research-objects') {
    ...
    push(candidates, 'researchObject', params.roId ?? params.id);
...
async function resolveCandidate(deps: JournalBoundaryDeps, candidate: Candidate): Promise<JournalTarget | null> {
  ...
  const lookup: Record<Exclude<TargetKind, 'workspace' | 'researchObject' | 'version' | 'agentSession' | 'agentTask' | 'toolApproval'>, {
    model: string;
    select: RecordValue;
