...
export const researchRecordOpenApi = {
  ...
  paths: {
    '/research/{publicId}/v/{versionNo}/artifacts/{artifactId}/download': { get: {
      summary: 'Download an attachment explicitly shared in this published version', security: [],
      parameters: [
        { name: 'publicId', in: 'path', required: true, schema: string },
      ...
      responses: {
        '200': { description: 'Frozen original bytes, streamed with Content-Disposition: attachment and Cache-Control: no-store.', content: { 'application/octet-stream': { schema: { type: 'string', format: 'binary' } } } },
        '404': { description: 'Publication unavailable, attachment unavailable, or frozen downloadAccess is not public. Legacy versions remain private.' },
        '503': { description: 'Published attachment storage is temporarily unavailable.' },
      },
    } },
