...
export function JournalAdminConsole() {
  ...
  const load = React.useCallback(async () => {
    setLoading(true); setLoadError(''); setSessionExpired(false);
    try {
      ...
      setLoadError(error instanceof ApiClientError && error.status === 403 ? adminT('forbidden') : adminT('loadFailed'));
    } finally { setLoading(false); }
  }, [adminT]);
  React.useEffect(() => { if (administrator) void load(); }, [administrator, user?.userId, load]);
