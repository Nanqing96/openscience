...
export function JournalManagementWorkbench({ journalId }: { journalId: string }) {
  ...
  async function addManual() {
    ...
    try {
      await createManualJournalArticle(journalId, {
        ...
        publishedDate: manual.publishedDate || undefined, journalTitle: data!.journal.nameZh, issns,
  ...
  return <div className="grid gap-8">
    <header className="border-b border-os-rule-paper pb-6"><p className="text-sm text-os-muted-paper">我的期刊 · {data.membership.role}</p><h1 className="mt-2 text-4xl font-normal">{data.journal.nameZh}</h1><p className="text-os-muted-paper">管理目录、来源、解读审核与固定版本。</p></header>
