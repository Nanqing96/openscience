...
export function ResearchContentManager({ researchObjectId, onChanged }: { researchObjectId?: string; onChanged?(item: { kind: string; resourceId: string }): void }) {
  ...
  async function load() {
    ...
    try {
      ...
      setItems((await apiRequest<{ items: ContentItem[] }>(path)).items);
  ...
  return <>
    ...
    <dialog className={`${styles.dialog} ${styles.wide}`} ref={dialog} aria-label={t('manageContent')}>
      ...
      {loading ? <p role="status">{t('loading')}</p> : error ? <p role="alert">{error} <button className={styles.action} type="button" onClick={() => void load()}>{t('retry')}</button></p> : items.length === 0 ? <p>{t('contentEmpty')}</p> : <ul className={styles.list}>
        {items.map((item) => <li className={styles.row} key={`${item.kind}:${item.resourceId}`}>
          <div><strong className={styles.itemTitle}>{item.title || t(`kind.${item.kind}`)}</strong><small>{t(`kind.${item.kind}`)} · {new Date(item.createdAt).toLocaleString(locale)}</small>{item.adopted && <small>{t('adopted')}</small>}</div>
          {item.canDelete ? <TrashActionButton kind={item.kind} resourceId={item.resourceId} title={item.title} onDone={() => { onChanged?.(item); void load(); }} /> : <small>{t('retained')}</small>}
        </li>)}
      </ul>}
    </dialog>
  </>;
