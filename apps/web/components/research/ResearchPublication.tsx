  ...
  const content = <div className="mx-auto max-w-4xl text-os-ink" data-workbench-publication="true">
    ...
    {ready && core && <>
      <article className="rounded-panel bg-white px-5 py-8 shadow-sm sm:px-8" data-publication-preview="true">
        ...
        <ScientificText as="p" className="mt-5 border-l-2 border-os-vermilion-ink pl-4 text-lg leading-8">{core.insight}</ScientificText>
        ...
        {SDF_FIELDS.filter((field) => field !== 'insight').map((field) => <section className="mt-7" key={field}><h3 className="research-section-title">{t(`fields.${field}`)}</h3><ScientificText as="p" className="mt-2 leading-8">{core[field]}</ScientificText></section>)}
      </article>
