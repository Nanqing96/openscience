...
skills_args=(run --rm --pull never --init --read-only --user 1000:1000
  ...
  --mount "type=bind,source=$skills_project_dir,target=/project,readonly"
...
case "$skills_operation" in
  list)
    ...
    exec docker "${skills_args[@]}" --network none openscience-development-skills:1.5.26 list
    ;;
  find)
    ...
    exec with-proxy docker "${skills_args[@]}" --network host \
      ...
      openscience-development-skills:1.5.26 find "$2"
    ;;
