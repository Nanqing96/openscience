...
verify_scansci_current() {
  ...
  run_remote "/usr/bin/node '$RELEASE_ROOT/infra/scripts/verify-scansci-mcp-runtime.mjs' --release-root '$RELEASE_ROOT' --release-sha '$RELEASE_SHA' --compose-file '$COMPOSE_FILE' --expected-mcp-image-id '$mcp_image_id' --require-worker '$require_worker' --require-oa '$require_oa_canary'"
}


verify_scansci_mcp_candidate() {
  local require_worker="${1:-1}"
  local require_oa="${2:-0}"
  ...
  run_remote "/usr/bin/node '$RELEASE_ROOT/infra/scripts/verify-scansci-mcp-runtime.mjs' --release-root '$RELEASE_ROOT' --release-sha '$RELEASE_SHA' --compose-file '$COMPOSE_FILE' --expected-mcp-image-id '$FINAL_SCANSCI_MCP_IMAGE_ID' --require-worker '$require_worker' --require-oa '$require_oa'"
}
...
verify_release_capability() {
  ...
  local source_sha256 package_freeze_sha256 model_manifest_sha256 scansci_deploy scansci_mcp_image_id
  ...
  scansci_mcp_image_id="$(read_capability_value "$file" scansci_mcp_image_id)" || return
...
run_remote "test -f '$PREVIOUS_CAPABILITIES_FILE'"
...
if [ "$PREVIOUS_CAPABILITY_SCHEMA" != 2 ]; then
...
FINAL_SCANSCI_MCP_IMAGE_ID="$(run_remote "docker image inspect --format='{{.Id}}' openscience-scansci-mcp:$RELEASE_SHA")"
...
verify_candidate_switch_contract() {
  local stage="$1" current_active current_worker_image current_parser_image current_scansci_mcp_image
  ...
  current_scansci_mcp_image="$(run_remote "docker image inspect --format='{{.Id}}' openscience-scansci-mcp:$RELEASE_SHA")"
  [ "$current_scansci_mcp_image" = "$FINAL_SCANSCI_MCP_IMAGE_ID" ] || {
    echo "错误：$stage 前 ScanSci MCP SHA tag 已被重定向" >&2
    return 1
  }
...
transaction_stop_candidate_scansci() {
  ...
  compose_current "rm -f -s scansci-mcp"
  run_remote "set -euo pipefail; cd '$RELEASE_ROOT'; containers=\$(XGS_RELEASE_ROOT='$RELEASE_ROOT' XGS_RELEASE_IMAGE_TAG='$RELEASE_SHA' docker compose --project-directory '$RELEASE_ROOT' --env-file '$PROD_ENV' -f '$COMPOSE_FILE' ps -a -q scansci-mcp); test -z \"\$containers\""
}
...
transaction_publish_capability_and_cas() {
  ...
  run_remote "set -e; install -d -m 0755 '$RELEASE_CAPABILITIES_DIR'; test ! -e '$CANDIDATE_CAPABILITY'; test ! -e '$CANDIDATE_CAPABILITY_STAGING'; (umask 022; set -C; printf 'schema=6\nembedding_deploy=%s\nbge_m3_enabled=%s\nmodel_version_id=%s\nmodel_revision=%s\nsource_sha256=%s\npackage_freeze_sha256=%s\nmodel_manifest_sha256=%s\nscansci_deploy=true\nscansci_mcp_image_id=%s\n' '$BGE_M3_DEPLOY_VALUE' '$BGE_M3_ENABLED_VALUE' '$BGE_M3_MODEL_VERSION_ID' '$BGE_M3_MODEL_REVISION' '$BGE_M3_SOURCE_SHA256' '$BGE_M3_PACKAGE_FREEZE_SHA256' '$BGE_M3_MODEL_MANIFEST_SHA256' '$FINAL_SCANSCI_MCP_IMAGE_ID' > '$CANDIDATE_CAPABILITY_STAGING')"
