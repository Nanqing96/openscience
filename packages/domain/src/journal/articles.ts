...
export async function txReleases(tx: JournalTx, articleId: string, publishedOnly = false) {
  ...
  return rows.map((r) => {
    ...
    return { id: r.id, revision: r.revision, versionNo: r.version.versionNo, publicId: r.version.researchObject.publicId, publishedAt: r.publishedAt, scope, url: `/research/${r.version.researchObject.publicId}/v/${r.version.versionNo}` };
  });
}
