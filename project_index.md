...
| `.Codex/troubleshooting/issues.json` | 项目工具/MCP/API/CI 故障的结构化问题库；issue-011–014 固化 noVNC client/channel/container-network 安全；issue-015 记录 bounded helper 后 stale forward，issue-016 记录 pinned `cache_dir/carsi_cookies` 快照契约；不记录账号、密钥、OAuth URL 或 token | 活文档 |
...
| `packages/database/src/ror-sync.ts` / `ror-sync-cli.ts` / `infra/migrations/20260902010000_ror_institution_directory/` | core migration 36 与 ROR schema-v2 importer：全球研究机构、注册域名、数据集版本及 credential/challenge 关联；250k/1GB 上限与批量事务 | **RELEASE CANDIDATE / NOT DEPLOYED**；本地 v2.12 导入 137,398/0 rejected，34,466 家含域名 |
...
| `infra/scripts/deploy.sh` / `infra/scripts/production-deploy-lock.mjs` / `infra/scripts/production-deploy-transaction.sh` / `infra/scripts/production-deploy-transaction-state.sh` / `infra/scripts/verify-document-parser-acceptance.mjs` / `infra/scripts/deploy.test.mjs` / `scripts/verify-embedding-runtime.mjs` | ECS immutable release：锁外只物化 exact archive；锁内单一远端 runner 持 FD9 完成 active/source/report/runtime/image、双账本、switch/health、expected-only CAS、公网验收与 durable-journal rollback；Compose entry 固定 exact release；schema 5 绑定 official MCP/auth exact image 与 rollback | **CURRENT DEPLOYED；PRODUCTION `7daff3f` / `ab290579`** |
| `apps/scansci-legal/` / `infra/scripts/{prepare-scansci-browser-network,scansci-browser-firewall,provision-scansci-secrets,verify-scansci-runtime}*` / `infra/systemd/{openscience-scansci-browser-firewall.service,docker.service.d/openscience-scansci-browser-firewall.conf,squid.service.d/openscience-scansci-browser-network.conf}` | 被官方 MCP 替代的私有 legal/browser/auth 实现与网络路径 | **REJECTED / INACTIVE ON PRODUCTION `7daff3f`**；仅为 rollback `ab290579` 与当前 Compose 插值保留，机构 journey 通过后精确删除 |
...
| `.taskmaster/` | task-master 任务状态；`optical-editorial-v3` 15/15 done；CURRENT `hermes-research-intelligence` 为 9/12，Tasks 1–9 done、Task 10 official-MCP production closeout | 工具自管；Task 11 blocked |
...
| `docs/specs/2026-09-02-scansci-upstream-mcp-design.md` | 上游 `v1.13.1` 官方 17-tool MCP、Worker 直连、持久数据/瞬态论文卷、`source_retrieval` provenance、ECS 正向验收与旧实现清理 | **CURRENT / PRODUCTION `7daff3f`**；institutional/four-entry/cleanup pending |
...
| `docs/plans/2026-08-30-hermes-external-retrieval-lifecycle-plan.md` | Taskmaster Task 10：Semantic Scholar/Tavily/ScanSci legal-only adapter、rights provenance、72h 私有缓存、10min 认证签名链接、租约 GC 与 ECS canary | **PARTIAL / TASK 10 REOPENED**；metadata/rights/temp lifecycle + ScanSci OA production，CARSI activation blocked by credential |
| `docs/plans/2026-08-30-scansci-default-capability-plan.md` | 旧版私有 ScanSci/browser protocol 三容器实施计划 | **DEPRECATED 2026-09-02 → `docs/plans/2026-09-02-scansci-upstream-mcp-plan.md`**；替代生产验收后删除 |
| `docs/plans/2026-09-02-scansci-upstream-mcp-plan.md` | 五步执行：ECS 官方 MCP 正向试点、`source_retrieval`、Worker MCP、官方登录/immutable deploy、生产验收后精确清理 | **CURRENT PLAN / TASK 5 STEP 3**；proxy candidate `263fc23` pending CI/deploy，ZJU Cookie pending |
| `apps/scansci-mcp/` / `apps/agent-worker/src/retrieval/scansci-mcp.ts` / `infra/scripts/{verify-scansci-mcp-runtime,scansci-auth-tunnel,prepare-scansci-auth-network}*` | 官方 `scansci-pdf==1.13.1` MCP/auth runtime、17-tool Worker client、durable acknowledge/discard、internal auth bridge 与 fixed-proxy browser wrapper | **PRODUCTION `7daff3f` / CANDIDATE `263fc23`**；OA/bridge green，proxy behavior/lifecycle review READY；CI/deploy/ZJU Cookie pending |
| `apps/scansci-legal/src/scansci_legal/{browser_protocol,limits}.py` / `apps/scansci-legal/test/test_browser_protocol.py` | Strict browser Task 3A：job/DOI proof envelope、POSIX pinned dirfd/O_NOFOLLOW、browser-output 独立验证、legal-input ack/cleanup、Cookie 原子发布与 100 MiB/8 KiB/4 KiB bounds | **LOCAL COMPLETE `2756952`**；109 pass / 7 platform skips / 0 fail，双复审 READY；Linux symlink gate pending CI/ECS |
| `apps/scansci-legal/src/scansci_legal/{source_guard,strict_browser,browser_worker}.py` / `apps/scansci-legal/test/{test_strict_browser,test_browser_worker}.py` | Strict browser Task 3B：pinned ScanSci AST/signature guard、build/start/runtime source-contract self-check、单次 direct Patchright、CDP 分块/累计有界且 exact-output 唯一 proof、180/210s 终态、串行 process-group、passive heartbeat、failure/ack/stale 与 pinned workspace cleanup | **PRODUCTION `405b85a`**；ScanSci 174 pass / 11 platform skips / 0 fail，institutional PDF proof pending valid credential |
| `packages/domain/src/{prisma-idempotency-conflict,workspace/helpers,research-object/research-objects,agent/agent,retrieval/literature-acquisition}.ts` / `apps/api/{src/routes/literature.ts,test/literature-acquisition.integration.test.ts}` | Task 5 统一文献 acquisition：active membership、system RO、Session、Task/credit/audit 共用 transaction primitives；单笔三次有界 Serializable 事务，exact replay 先于余额；P2002 精确绑定 model/field/constraint；Redis commit 后投递与 pending recovery | **LOCAL CANDIDATE `4763228`**；Domain/API/Agent 本地门禁 green，两连接 PostgreSQL SSI race/mismatch suite 已加入但按本地 no-Docker 规则未执行；ECS pending |
| `packages/domain/src/retrieval/provider-state.ts` / `packages/domain/test/retrieval/provider-state{,.integration}.test.ts` / `packages/domain/{tsconfig.integration.json,vitest.integration.config.ts}` / `apps/agent-worker/{src/index.ts,src/retrieval/orchestrator.ts,test/gateway-config.test.ts,test/retrieval/{handler,orchestrator}.test.ts}` / `infra/migrations/20260830020000_scansci_provider_state/` | Task 6 PostgreSQL-authoritative ScanSci 状态：generation、audit/current-admin notification 原子事务、三次 P2034、重放/回滚、raw success clear、descriptor-only Secret 与 disabled rollback；真实 PG forward/rollback/redeploy + 双 client 并发合同 | **LOCAL CANDIDATE `60b3740` / REVIEW READY**；Domain 520、Database 26、Worker 501，集成合同仅 typecheck；migration 33/ECS execution pending |
| `apps/web/components/dashboard/LiteratureAcquisition.tsx` / `apps/web/lib/{api,literature-acquisition-state}.ts` / `apps/web/app/dashboard/page.tsx` / `apps/web/test/{literature-acquisition-state,e2e/auth-dashboard.spec}.ts*` / `packages/domain/src/{agent/agent,retrieval/{browser-result,retrieve-payload,literature-acquisition}}.ts` / `packages/domain/test/helpers/retrieval-payload-parity.ts` / `apps/api/{tsconfig.integration.json,test/literature-acquisition.integration.test.ts}` / `apps/agent-worker/src/retrieval/handler.ts` | ScanSci plan Task 7 Personal Space acquisition：server-only durable marker、shared `canRetry`/authority predicate、P2034-only Serializable retry、单次 ID-only exact recovery、storage-labelled/terminal-distinguishable parity、poll/reconcile、44px/inert link 与临时下载 | **LOCAL TASK 7 REVIEW CANDIDATE `185d5d6`**；Domain/API/Worker `534/99/502` + real-PG contracts typecheck，既有 7 条 component journey；ECS 375px/one-use download pending |
| `apps/web/lib/hermes/literature-intent.ts` / `apps/web/components/hermes/HermesAssistantDrawer.tsx` / `apps/web/app/research-objects/[id]/{hermes,files}/page.tsx` / `apps/web/components/intake/EvidenceIntake.tsx` / `apps/web/test/{hermes-literature-intent,literature-acquisition,surface-shells}.test.tsx` / `.superpowers/sdd/2026-08-30-scansci-default-capability-plan/task-8-report.md` | ScanSci plan Task 8 四入口 + fix rounds：explicit intent key/fingerprint 先于 generic history exact replay、route-authoritative RO、durable target、API/Domain pre-limit recovery、IME-safe disclosure；复用 Task 7 poll/retry/download，无 provider/auth controls | **LOCAL TASK 8 REVIEW CANDIDATE `c0bcf0a`**；Domain/API/Worker/Web `535/101/502/464 + 5 Node`、browser A-vs-B `5/5`、product `72/72`、Hermes `19/19 + 8/8` green；ECS 375px/CARSI/OA pending |
| `apps/scansci-legal/src/scansci_legal/{limits,main,upstream,upstream_worker,oa_canary}.py` / `apps/scansci-legal/test/{test_upstream,test_oa_canary}.py` / `apps/agent-worker/src/retrieval/scansci.ts` / `infra/compose/docker-compose.{dev,prod}.yml` / `infra/scripts/verify-scansci-runtime.mjs` / `docs/security/production-security-checklist.md` / `.superpowers/sdd/2026-08-30-scansci-default-capability-plan/task-9-report.md` | descriptor Secret、NAT64、serial/negative cache、100 MiB RLIMIT、internal controlled egress 与真实 arXiv HTTP→worker `open_access` PDF canary；sidecar-independent prepublication、exact Worker env、locked publish/rollback cleanup | **PRODUCTION `cca5908` / NOFILE FIX `396b301` LOCAL**；OA PDF 24,671,920 bytes，CARSI/四入口 pending |
| `apps/agent-worker/src/retrieval/` / `apps/api/src/routes/temporary-documents.ts` / `packages/domain/src/retrieval/` / `infra/migrations/20260830010000_external_retrieval_lifecycle/` / `packages/storage/src/minio-adapter.ts` | Task 10 provider-normalization、rights、tenant-bound task、72h temp object、600s one-use cookie、租约 GC、SeaweedFS checksum metadata 兼容 | **CURRENT PRODUCTION `7daff3f`**；Semantic Scholar 与 official ScanSci OA、PDF hash/download/replay/GC/provenance 全绿；institutional/four-entry pending |
...
| `docs/handoff/2026-08-16-hermes-2d-pet-handoff.md` | 唯一 compact CURRENT 交接：Research Intelligence 生产基线、官方 ScanSci MCP、磁盘卫生、固定约束与下一动作 | **CURRENT active-memory**；production `7daff3f`、rollback `ab290579`；ZJU Cookie/institutional PDF/cleanup pending |
...
| `docs/runbooks/deployment.md` | 部署 runbook（cloud-sync/迁移/seed/nginx/验证、Parser/BGE/ScanSci 隔离、Windows Git Bash）；§5.49–5.63 记录旧 ScanSci recovery、官方 MCP 部署、auth bridge hotfix 与清理顺序 | **CURRENT**；production `7daff3f` / `ab290579`，official MCP/auth bridge/parser fix production |
...
| `infra/migrations/` | Prisma 迁移 1–36（36 = ROR 全球机构目录；35 = 学术身份凭证/机构邮箱挑战/多作用域角色；34 = ScanSci `source_retrieval` provenance），各附 rollback.sql | **RELEASE CANDIDATE 36；PRODUCTION 34/34**；search 2/2 |
| `infra/schema.prisma` | core Prisma schema：既有平台模型 + Research Intelligence/Identity/Reading/External Retrieval/ScanSci provider state scoped relations + `source_retrieval` rights basis + academic credentials/scoped roles/ROR directory | **RELEASE CANDIDATE migration 36；PRODUCTION migration 34** |
