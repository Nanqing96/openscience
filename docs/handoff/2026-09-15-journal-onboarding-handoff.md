...
- 当前线上 release `48814fe00640022ea188c45067994fda60508939` / rollback `3906b93d5fa3c911f3f2441ac6a6f28cf7dda5ec`。2026-09-16 canonical 发布成功并完成公网精确版本核对；后续操作重新定锚。历史生产版本保留。
...
- 退回修改闭环已部署/推送：明确三种审核操作、拒绝二次确认、管理员误拒更正、同编号继续修改/保存/重提。更正要求管理员、rejected/未入驻、匹配修订号及必填原因，保留原审核意见和资料，同事务审计。独立 high GO；必要服务器构建/启动、精确版本切换与事务清除成功，无迁移/测试/CI；日志 `.work/journal-revision-workflow-deploy.log`。
- 真实 Edge 从研究桌面点击期刊审核后，按用户授权将指定误拒申请更正为待修改；已观察处理中/防重、成功提示，原编号及审核意见保留。回执实际点击“继续修改”进入原表单，字段、保存修改/重新提交按钮可用，邮箱及原资料截图回填正确；后台定向只读确认 rejected 与 reopened 两条历史事件均在。未代填、保存或重提资料，未批准期刊；拒绝二次确认/重提写入仅静态复核与构建，不声称已实写验收。
...
1. 退回修改已上线并完成用户授权的误拒更正；用户可在“我的申请 → 继续修改”完成英文修订后重新提交。后续仅按用户授权处理审核，不自动代填或批准，不新建虚构期刊。
