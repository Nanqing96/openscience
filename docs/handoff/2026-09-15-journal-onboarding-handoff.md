...
- 当前线上 release `7af30d8e22423815fca27aa70df0cc570dcc25c2` / rollback `3cc5bedb4c288d692473f27d1c1e5bdc18130f64`。2026-09-16 canonical 发布成功，公网 `/__release` 精确匹配；后续操作重新定锚。历史生产版本保留。
...
- 新修订待发布：三个站内管理员入口、当前账号角色返回、管理页加载/登录/权限/重试状态已实现；仅期刊页面和 API 改为 Nginx 内部实时会话/角色检查，管理 API 权限/CSRF/审计保持，ADR-003 已登记用户明确批准的例外。当前无迁移，静态 high 审查及真实点击观察待完成。
...
1. 完成本轮入口与认证衔接独立审查后 canonical 发布；从现有管理员真实会话点击站内“期刊审核”观察指定申请，同时核匿名页面登录跳转和 API 拒绝。禁止代为审批或新建申请。
