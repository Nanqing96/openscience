...
- 当前线上 release `aee3f3184e6813223ca3f361922ec30b21b54fe3` / rollback `7af30d8e22423815fca27aa70df0cc570dcc25c2`。2026-09-16 canonical 发布成功，公网 `/__release` 精确匹配；后续操作重新定锚。历史生产版本保留。
...
- 最新反馈“通过无反应”：只读确认指定申请仍 submitted，四项元数据为中文；旧 Console 把失败提示放整页末尾且无 pending。候选修复为独立申请审核卡：共享英文前置提示、本行聚焦反馈、提交中/防重、即时应用返回状态及保留结果的后台刷新；补件说明按钮只填表，不提交。无后台规则/生产数据修改，待 high 复核与发布后定向点击观察。
...
1. 发布审核反馈修订并真实观察：只点击既有中文资料会被前置校验拦截的“通过”，核对即时错误/字段提示；不代为补件或通过。旧中文资料仍需正常补件再修改，不新建虚构期刊。
