...
## 2026-09-13 生图稳定性修复（历史证据，执行器仍适用）
- 登录与7891→Squid→7890代理有效；旧页挂起曾阻塞全部attach，精确关闭后恢复219ms。ac113键盘输入/Control+End/picture_v2修复已部署；40d64/1049/9c2均提交前失败，不重放。
- b78原会话唯一主图DOM→同源estuary下载（无redirect、PNG、30MiB限额），仅允许原deadline+1h取已有结果；completed-result回收保留late/uncertain。5260于08:10Z单次提交、08:29:23Z恢复入库，987834B/1672×941→1280×720；质量不足仍draft，86ffe为后续候选。收据/jobs/hermes-quantization-image-{product-recovery,product-result,gallery-reading}-20260913.json。
- 501自有page生命周期以browser instance+target+job精确判断，保护已提交未完成及未知页；安装image→science→shared三锁。d369补有界异步close确认，未复演新页；实际/jobs/hermes-completed-image-page-cleanup-20260913.json，独立High PASS，不声称任意Chrome卡死可自愈。
- 回滚/opt/openscience-chatgpt-browser/patches/<commit>/{before,deployment.json}；501回退两runner、d369回退helper。交付tmp/{image-download-fix-deploy,page-lifecycle-deploy,page-close-confirm-deploy}.sh；timers恢复active，无测试/CI。完整历史见Git c3d20046此节，当前UI与版本以本文末节为准。
