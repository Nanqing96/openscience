...
- 已实现首页/研究桌面/期刊目录的管理员专用入口及管理页加载、登录、权限、重试提示。用户明确批准仅期刊审核改用网站管理员登录；ADR-003 例外与运行手册同步，其余后台 Basic Auth 保留。待独立审查、必要服务器发布和真实点击到申请列表；状态与精确版本见[期刊 CURRENT](handoff/2026-09-15-journal-onboarding-handoff.md)。
