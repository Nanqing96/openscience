...
- 已实现英文必填/中文选填、后台一致校验、英文优先展示、服务说明及移除专业审核选项；本人申请状态、补件编辑、真实编号和成功跳转回执已接入。新增差异独立 high GO，待必要服务器构建和真实页面观察。
- 只读核实用户指定账号及指定期刊申请的权限/状态；没有变更管理员或审核申请。版本和部署证据仅见 [期刊 CURRENT](handoff/2026-09-15-journal-onboarding-handoff.md)。
