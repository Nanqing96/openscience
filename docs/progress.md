...
## 2026-09-16 — 期刊生产整合，已恢复授权部署

- 用户授权上线后，发现原期刊代码基线落后于真实生产；已保留发布清单哈希一致的两次生产增量，整合独立公开编号、冻结身份、回收站与检索权限、上传清理锁。新增差异独立 high 静态审查通过；没有运行测试、CI或本机构建。
- 用户最新授权现在上传，若并行部署未结束则监控等待；实际发布锁已空闲、线上源已整合，双数据库备份成功。修复 Windows 上传入口后已取得锁并开始服务器构建，迁移/切换结果待记录。精确 branch/HEAD/release/rollback、历史验收范围和续作步骤仅见 [期刊 CURRENT](handoff/2026-09-15-journal-onboarding-handoff.md)。
