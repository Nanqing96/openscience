...
>
> 2026-09-22 增量范围：在现有 `codex/journal-onboarding` 上优先实现来源与版权矩阵、AI 加工优先级、服务包额度页面。下述原首期需求继续有效；本轮增量约束见第 18 节。Topic Hub 和机器访问分析仍为第二批。
...
| 商业方向 | 基础入驻免费；批量 AI 加工、专业审核、持续运营及监测收费 |
| 首期用户 | 3–5 家共建期刊及其编辑部 |
...
| P1，第二阶段 | 大规模历史回溯、CSV/XML 集成导入、作者结构化核对、完整 Topic Hub、外部 AI 监测与报告、付费专业审核服务流 |
...
| 字段 | 要求 | 公开性 |
|---|---|---|
| 中文/英文刊名 | 至少填写一种，支持原文名称与别名 | 公开 |
| eISSN / pISSN | 至少一个；没有时进入人工特殊审核，不能虚构 | 核验后公开 |
| 原官网 | 必填，填写可确认的期刊页面 | 公开 |
| 主办/出版单位 | 必填；两者不同时分别记录 | 公开 |
| 学科、简介、Logo | 学科必填，其他可稍后补齐 | 公开 |
| 申请人姓名、职务、邮箱 | 必填并验证账号邮箱 | 默认不公开 |
| 代表编辑部的依据 | 单位邮箱、官网可核对身份、授权材料等 | 私有 |
...
## 18. 2026-09-22 增量需求：来源、优先级与服务额度

### 18.1 来源与版权矩阵

逐篇记录多项来源材料的类型、来源地址、内容范围、权限依据、可信状态及变更记录。明确当前实际用于加工的来源；补充材料的授权不能扩大主来源的授权。旧论文继续沿用原有来源和五项独立权限，不因升级而自动获得新权限。

由服务端计算可生成与可公开范围及阻断理由。书目不能生成解读；摘要只能生成摘要范围的解读；全文访问或许可名称本身不等于处理及公开授权。原图复用与文字图义说明分别表达，未提供且未核验的科学信息不推断为已具备。来源冲突、失效、撤销或明确受限不能被人工优先级覆盖。

来源与权限变化继续使用修订号和审核摘要防冲突，旧审核失效；入队、执行和结果保存均复核当前权限。已发布内容的访问收紧同样适用于固定页面、公开 API 和站点地图，不将内部依据或私有材料放进公开响应。

### 18.2 AI 加工优先级

提供可解释的 0–100 分和 P0/P1/P2/P3/blocked 等级。评分包括来源完整度（20）、权限清晰度（15）、近期性（10）、展示价值（15）、学术中心性（10）、解析状态（10）、编辑指定（10）、主题匹配（10）。缺乏证据的维度明确为未知或待编辑确认，不编造引用量、影响力或解析成功率。

编辑可查看各维度和建议动作、浏览排序后的论文、标记重点、延后或恢复处理，并显式发起加工。优先级只帮助选取论文；提交作业仍复用身份、来源、当前状态、并发限制、幂等和额度预占检查。仅打开页面或改变分数不自动消耗额度；不能把排序分数作为科学质量评价或付费公开排名。

### 18.3 服务包与额度页面

显示服务方案、实际有效批次、可用余额、在途预占、已消耗、已过期、到期时间、账本记录和存储使用。可用额度按未过期批次的 `remaining - reserved` 计算，过期批次中的在途预占单列，避免把账本的 remaining 字段误显示为可立即使用的余额。

展示 Free、Starter、Pro、Premium、Custom 服务意向，由负责人或管理员提交申请、平台报价并人工开通。申请意向不等同生效套餐；不虚构价格、周期额度、自动续费或第二批功能的已交付权益。编辑可查看可用额度，审核员不获得商务资料访问权。

本轮保留 BIZ-03 的既有整数额度规则：一次成功交付的 AI 草稿消耗 1 篇，失败/取消释放预占，技术重试不重复消耗；人工编辑、审核及固定版本发布不新增收费。聊天增强方案中的小数额度、多计量账本、自动周期续费须另行确认商务规则与数据迁移后实施。

### 18.4 本轮增量验收

| 场景 | 预期 |
|---|---|
| 旧论文首次打开矩阵 | 原来源和权限准确呈现，没有隐式扩权 |
| 增加辅助来源、主来源仍受限 | 辅助授权不能解除主来源阻断 |
| 权限变更与旧修订并发保存 | 旧修订冲突，人工审核须针对新修订 |
| 高分、重点标记与无权来源同时存在 | 无权来源仍不能入队或发布 |
| 期刊 A 成员或未指派审核员访问期刊 B | 拒绝私有矩阵、优先级和服务数据 |
| 额度到期前后读取服务页 | 可用额立即反映到期，预占和历史消耗保留 |
| 入队、失败/取消、重新查看额度 | 预占和释放与既有账本一致，无重复扣费 |
| 桌面和手机完成三个新增入口 | 可读、可操作，错误可见，无页面横向溢出 |

Topic Hub、机器访问分析、多刊统一额度、在线支付、真实生产模型和解析器验收不计入本轮完成声明。实现证据与未完成项以 CURRENT handoff 为准。
