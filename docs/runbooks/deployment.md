...
本次结果：服务器完整构建、两个核心迁移及服务启动完成，精确公网release为ef9e6e97；未运行测试/预检/CI。旧API/Worker收到停止请求后未退出；只读确认 running AgentTask 为空后，对这两个旧容器执行 `docker stop --time 10`，部署事务继续完成，未触发删除或新科研任务。只证明本次停止阻塞已解除，不宣称信号处理根因已修复。首发rollback标记c0bc653d不兼容，后续必须选同样包含新生命周期的release；最新版本对见CURRENT。
