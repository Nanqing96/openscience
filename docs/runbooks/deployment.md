...
2026-09-14配图语义审阅候选：现实际应用89d05d6d、rollback b2f3cf37、Chat bundle e2。新review request v2只用于配图，旧v1保持；发布须先物化精确候选、服务器安装锁定依赖并构建 @openscience/ai-gateway...，用该release的Chat installer与既有renderer镜像更新receiver，再部署应用。回退应用到89d05时可保留兼容v1的新版receiver；若要回退receiver，先回退应用，禁止旧receiver接新v2任务。按既有no-tests/skip-migrate部署，不运行预检或演练；最新结果见CURRENT handoff。
