...
本轮范围：实际交付树相对 244726a4 的代码/规则差异，以及受其影响的当前文档入口。以下是定向静态诊断，不是全仓无债证明或量化健康评分；未运行扫描、测试、模型或应用构建。
...
| 候选 illustration-review.ts 全稿重写，且 composition 接受任意文本；planner 拆分旧画面时依赖“科学编码：…；排布：…” | 审阅可能改掉科学焦点，丢分隔后下次改图又重走科学分析 | 未解决、未部署；恢复配图前收敛审阅改动范围并复用科学/艺术表示，不能只加提示词掩盖 |
| 上游已审语义未充分传给配图（见上一节） | 重读长 Claim/整批 Evidence，再造科学判断 | 未解决；与上述审阅问题同批处理，保留来源条件与用户批准，不硬套单篇模板 |

三文件代码去重经独立 High 静态复核：来源/权限/并发重验保持，合并顺序与输入隔离确认；未构建、未部署，不声称运行通过。治理和这些小重构不能把原有科学质量欠缺变成“已完成”。
