...
|---|---|---|
| 期刊入驻与 AI 解读 | `apps/web/app/journals/`、`apps/api/src/routes/journals.ts` → `packages/domain/src/journal/` → `apps/agent-worker/src/journal-worker.ts`，复用既有 Gateway/Parser；`form-contract.ts` 共享英文元数据和服务规则 | 已部署：申请/人工核验、权限与许可、持久化作业和额度、审核及固定公开版本；真实页面已查看本人申请回执、编号与状态，英文输入和学科空格已定向观察，私有 API 匿名 401。旧中文申请需正常补件；未新建虚构申请验证提交。尚无真实期刊模型/解析端到端产物，不声称提升引用；版本/证据独立见[期刊 CURRENT](../handoff/2026-09-15-journal-onboarding-handoff.md) |
