...
|---|---|---|
| 期刊入驻与 AI 解读 | `apps/web/app/journals/`、`apps/api/src/routes/journals.ts` → `packages/domain/src/journal/` → `apps/agent-worker/src/journal-worker.ts`，复用既有 Gateway/Parser；共享英文规则，`JournalAdminLink.tsx`/`JournalApplicationReviewCard.tsx` 提供审核入口与反馈 | 已部署申请、人工核验、权限/许可、作业/额度及固定公开版本；期刊审核网站登录例外经用户明确批准，其他后台原认证保持。退回修改/拒绝确认/误拒更正已上线，用户指定申请经真实界面更正为待修改，原编号/审核历史保留，继续修改入口实际可用；未代填、重提或批准。尚无真实期刊模型/解析端到端产物，不声称提升引用；版本/证据独立见[期刊 CURRENT](../handoff/2026-09-15-journal-onboarding-handoff.md) |
