...
## 产品落地优先：强约束（用户明确要求，更新至 2026-09-11）
- UI改动先按项目`.agents/skills/frontend-design/SKILL.md`及其中引用的设计skills执行；阅读顺序是贡献→核心图/视频→精炼正文→折叠资料。Hermes是主要操作入口，内部记录不置于成果之前。用户2026-09-11授权本次服务器生图、科学审核及公开发布的真实完整流程，不扩展为全套测试或本机运行。
...
- 2026-09-10 最新指令：高级解析、Hermes论文分析Skill、有界并发和多能力协同必须实际投入生产，并用真实论文验证完整结果；测试只服务于本次产品能力和生产风险，不扩建测试工程或用检查代替交付。
- 用户2026-09-12要求优先借鉴现有和GitHub成熟科研Skill/实现：先核上游代码与实际运行调用，区分已接入、方法借鉴和未采用，再补具体缺口，避免重造工具或盲目堆提示词。连续局部修补、重复失败或方向与目标不符时停止重试，保留稳定状态并说明证据、偏差与下一步；复用有效来源/阶段结果，质量以真实产物核对为准，不以部署结束。安装第三方能力仍须明确授权。

- 第一优先级是把用户需要的服务器产品功能实现并部署，不得用预检、测试、评审工具建设或重复分析替代交付。
- 默认不执行无关全套回归；对新解析器、并发、权限、证据、模型编排和清理操作执行能够证明真实产品结果与回滚安全的最小必要检查。
